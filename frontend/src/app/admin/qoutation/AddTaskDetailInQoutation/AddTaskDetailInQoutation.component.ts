import { Component, Injector, EventEmitter, ViewChild, Output, ElementRef } from "@angular/core";
...
@Component({
  selector: "AddTaskDetailInQoutation",
  templateUrl: "./AddTaskDetailInQoutation.component.html",

})
export class AddTaskDetailInQoutationComponent extends AppComponentBase {
  ...
  Capture() {
    let elemenent = document.getElementById('content2convert');


    return new Promise(resolve => {
      html2canvas(elemenent, {
        scale:1
      }).then(canvas => {
      var imgWidth = 208;
    ...
    //   var imgHeight = canvas.height * imgWidth / canvas.width;
    //   let contentDataURL = canvas.toDataURL('image/png');
    //   document.write('<img src="'+contentDataURL+'"/>');
