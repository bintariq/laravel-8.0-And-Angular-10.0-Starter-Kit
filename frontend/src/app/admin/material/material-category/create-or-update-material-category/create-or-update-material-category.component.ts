...
@Component({
  selector: 'app-create-or-update-material-category',
  templateUrl: './create-or-update-material-category.component.html',
  styleUrls: ['./create-or-update-material-category.component.scss']
})
export class CreateOrUpdateMaterialCategoryComponent extends AppComponentBase {
  ...
  constructor(injector: Injector, private config: ConfigService) { 
    ...
    if (this.params) {
      ...
      this.config.get('show-material_category/'+this.params+'').subscribe((data) => {
  ...
  onCreateCategory(){
    ...
    this.config.post('add-material_category', dat).subscribe((data)=>{
      if(data.success=="0"){
        this.toastr.success('Category Already Exist!');
      }
      else{
        this.toastr.success('Category Added Successfully!');
        this.router.navigate(['/admin/material/material-category']);   
      }        
    });
  ...
  onEditCategory() {
    ...
    this.spinnerService.show();
    this.config.post('edit-material_category', dat).subscribe((data) => {
      console.log(data);
      if (data.success == 0) {
  ...
  getParentCategories(){
    this.config.get('fetchmaterial_category/'+parent).subscribe((data) => {
