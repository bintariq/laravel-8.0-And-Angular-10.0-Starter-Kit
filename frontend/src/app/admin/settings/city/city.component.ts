...
export class CityComponent extends AppComponentBase {
  ...
  setPage(pageInfo) {
    ...
    this.config.get('fetchCities?page=' + pageNum + '').subscribe((data) => {
  ...
  searchRecord(event) {
    ...
    if (searchText) {
      this.spinnerService.show();
      this.config.get('fetchCities?searchText=' + searchText + '&searchColumn=' + this.searchColum).subscribe((data) => {
        this.page.totalPages = data.last_page;
        this.page.totalElements = data.total
        this.rows = data.data;
        this.spinnerService.hide();
  ...
  delete(id) {
    this.spinnerService.show();
    this.config.get('deleteCity/' + id).subscribe((data) => {
      this.setPage({ offset: 0 });
