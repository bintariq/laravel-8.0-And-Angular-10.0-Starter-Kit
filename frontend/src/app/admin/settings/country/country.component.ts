...
export class CountryComponent extends AppComponentBase {
  ...
  columns = [
    ...
    { prop: 'name', name: 'Name' },
    { prop: 'code', name: 'Code' },
    { prop: 'wrench_time', name: 'Wrench Time' },
  ...
  setPage(pageInfo) {
    ...
    this.config.get('fetchCountries?page=' + pageNum + '').subscribe((data) => {
  ...
  searchRecord(event) {
    ...
    if (searchText) {
      this.spinnerService.show();
      this.config.get('fetchCountries?searchText=' + searchText + '&searchColumn=' + this.searchColum).subscribe((data) => {
        this.page.totalPages = data.last_page;
        this.page.totalElements = data.total
        this.rows = data.data;
        this.spinnerService.hide();
  ...
  delete(id) {
    this.spinnerService.show();
    this.config.get('deleteCountry/' + id).subscribe((data) => {
      this.setPage({ offset: 0 });
